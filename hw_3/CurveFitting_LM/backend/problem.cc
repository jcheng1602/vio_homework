...
bool Problem::Solve(int iterations) {
    ...
    TicToc t_solve;
    // Record record_lambdas;
    Record record_vertexs;
    ...
    while (!stop && (iter < iterations)) {
        ...
        // record_lambdas.add(iter,currentLambda_);
        ...
        while (!oneStepSuccess)  // 不断尝试 Lambda, 直到成功迭代一步
        {
            ...
            //误差上升的时候,false,继续优化迭代;误差下降的时候,true,跳出循环记录下来
            oneStepSuccess = IsGoodStepInLM();//可能会出现rho无异常,跳出一次while,改变lambda
...
//                stop = (b_max <= 1e-12);
            ...
            // record_lambdas.add(lambda_iter,currentLambda_);
            lambda_iter++;
            for(auto vertex: verticies_){
                std::cout<<"vertex:"<<vertex.second->Parameters();
                Vec3 abc = vertex.second->Parameters();
                record_vertexs.add(lambda_iter,abc(0),abc(1),abc(2));
            }
        }
        iter++;
...
bool Problem::IsGoodStepInLM2() {
    ...
    double alpha = frac1 / (((tempChi - currentLambda_)/2) + 2*frac1);
